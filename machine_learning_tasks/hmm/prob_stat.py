...
def saveCwsInfo(path, cwsInfo):
    '''保存分词训练数据字典和概率.cws:Chinese Word Segmentation'''
...
def sent2vec2(sent, vocab, ctxWindows = 5):
    '''
    句子转向量
    获取输入句子 sent 的所有长度为 ctxWindows 的序列的短语向量
    ----------------------------------
    params:
        sent: (sentence) 句子（句子的单词列表）
        vocab: vocabulary 词汇表
    return X(list): 返回所有窗口大小为ctxindows 的短词序列
    '''
    charVec = [] # 列表元素: vocab[char]
    for char in sent:
        if char in vocab:
...
def doc2vec(fname, vocab):
    ...
    tagCnt = [0 for i in range(tagSize)] # [0, 0, 0, 0]
    # 4 x 4 数组
...
def genVocab(fname, delimiters = [' ', '\n']):
    ...
    # 加入未登陆新词和填充词
    # 把 retain-unknown 和 retain-padding 的 index 放最后两个
    ...
    #返回字典索引{word: index} 与 单词列表（index顺序与字典一致）
